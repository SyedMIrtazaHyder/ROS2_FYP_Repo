...
class GazeboBEV(Node):
    def __init__(self):
        super().__init__('gazebo_bev')
        self.boundary = {
            "minX": -25,
            "maxX": 25,
            "minY": -25,
        ...
        self.points = Subscriber(self, PointCloud2, "/lidar/points")

        queue_size = 10
        max_delay = 0.05
        self.time_sync = ApproximateTimeSynchronizer([self.points],
                                                     queue_size, max_delay)
        self.time_sync.registerCallback(self.publishBEV)

    def publishBEV(self, points):
        ...
        xyzi = np.array([[*point] for point in point_cloud2.read_points(points, field_names=['x', 'y', 'z', 'intensity'])], dtype=np.float32).reshape(1024, 64, 4).transpose(1,0,2)
        ...
        #bev = bev[:, ::-1] # Required when using the LUT table for conversion 
...
def main():
    ...
    gazebo_bev = GazeboBEV()

    rclpy.spin(gazebo_bev)

    gazebo_bev.destroy_node()
    rclpy.shutdown()
